...
namespace Library {
    ...
    void FontManager::init()
    {
        ...
        shaders.push_back(ShaderDefinition(GL_VERTEX_SHADER,FontVstr));
        shaders.push_back(ShaderDefinition(GL_FRAGMENT_SHADER,FontFstr));
        ...
        // Load font as face
        FT_Face face;
        if (FT_New_Face(ft, FontTTFstr, 0, &face))
    ...
    void FontManager::renderText(std::string text,GLfloat x,GLfloat y,GLfloat scale,glm::vec3 color)
    {
        ...
        projection = mProjViewMat;
        glUniformMatrix4fv(glGetUniformLocation(mShaderProgram.Program(), "projection"), 1, GL_FALSE, &projection[0][0]);
        glUniform3f(glGetUniformLocation(mShaderProgram.Program(), "textColor"), color.x, color.y, color.z);
        ...
        {
            FONT::Charactar ch = Characters[*c];
